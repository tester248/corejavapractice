...
//		numbers.stream().distinct().forEach(System.out::println); //distinct in intermediate
		
		System.out.println(numbers.stream().distinct().count()); // count is terminal operator
		
		System.out.println(numbers.stream().filter(n -> n > 5).count()); // filter
		
		System.out.println(numbers.stream().distinct().sorted().forEach(System.out::println)); // sorted
		
		System.out.println(numbers.stream().reduce(0, (n,m) -> n + m)); //reduce
